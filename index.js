...
async function getRecipesByFirstLetter(letter) {
  resultsGrid.innerHTML = "";
  showMessage(`Showing recipes starting with "${letter}"`, false, true);
...
async function getRecipeDetails(id) {
  if (!id) {
    modalContent.innerHTML = '<p class="message error">Invalid recipe ID.</p>';
    showModal();
    return;
  }
  ...
  try {
    ...
    modalContent.innerHTML =
      '<p class="message error">Failed to load recipe details. Please check your connection or try again.</p>';
  }
}
...
function displayRecipeDetails(recipe) {
  ...
  const youtubeHTML = recipe.strYoutube
    ? `<h3>Video Recipe</h3><div class="video__wrapper"><a href="${
        recipe.strYoutube
      }" target="_blank">Watch on YouTube.</a></div>`
    : "";
  const sourceHTML = recipe.strSource
    ? `<div class="source__wrapper"><a href="${recipe.strSource}" target="_blank">View Original Source.</a></div>`
    : "";
